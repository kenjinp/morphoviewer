...
var THREE$1 = require('three-canvas-renderer');
var ObjParser = _interopDefault(require('parse-wavefront-obj'));
require('canvas');
...
class ThreeContext extends EventManager {
  ...
  constructor(canvas$$1) {
    ...
    // init camera
    this._camera = new THREE$1.PerspectiveCamera(27, 1, 1, 1000000);
    ...
    // adding some light
    const light1 = new THREE$1.DirectionalLight(0xffffff, 0.8);
    ...
    this._renderer = new THREE$1.CanvasRenderer({
      canvas: this._canvas,
      alpha: true
  ...
  addObjToMeshCollection(objStr, options) {
    ...
    geometry.setIndex(new THREE$1.BufferAttribute(indices, 1));
    geometry.addAttribute(
      "position",
      new THREE$1.BufferAttribute(positions, verticesPerPolygon)
    );
    geometry.computeBoundingSphere();
...
class MorphoViewer {
  constructor(canvas$$1) {
    this._threeContext = new ThreeContext(canvas$$1);
  }
