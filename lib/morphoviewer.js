...
const TrackballControls = function (object, domElement) {
  ...
  const _movePrev = new THREE$1.Vector2();


  const _moveCurr = new THREE$1.Vector2();


  const _lastAxis = new THREE$1.Vector3();


  let _lastAngle = 0;


  const _zoomStart = new THREE$1.Vector2();


  const _zoomEnd = new THREE$1.Vector2();
  ...
  const _panStart = new THREE$1.Vector2();


  const _panEnd = new THREE$1.Vector2();
  ...
  this.rotateCamera = (function () {
    ...
    const eyeDirection = new THREE$1.Vector3();


    const objectUpDirection = new THREE$1.Vector3();


    const objectSidewaysDirection = new THREE$1.Vector3();


    const moveDirection = new THREE$1.Vector3();
  ...
  this.panCamera = (function () {
    ...
    const objectUp = new THREE$1.Vector3();


    const pan = new THREE$1.Vector3();
...
STLLoader.prototype = {
  ...
  load(url, onLoad, onProgress, onError) {
    ...
    const loader = new THREE$1.FileLoader(scope.manager);
  ...
  parse(data) {
    ...
    function ensureString(buffer) {
      if (typeof buffer !== 'string') {
        return THREE$1.LoaderUtils.decodeText(new Uint8Array(buffer))
      }
...
class Tools {
  ...
  static makeCylinder(vStart, vEnd, rStart, rEnd, openEnd) {
    ...
    const offsetPosition = new THREE$1.Matrix4();// a matrix to fix pivot position
    offsetPosition.setPosition(position);

    const cylinder = new THREE$1.CylinderBufferGeometry(rStart, rEnd, distance, 8, 1, openEnd);
    const orientation = new THREE$1.Matrix4();// a new orientation matrix to offset pivot
    orientation.multiply(offsetPosition); // test to add offset
    const offsetRotation = new THREE$1.Matrix4();// a matrix to fix pivot rotation
    orientation.lookAt(vStart, vEnd, new THREE$1.Vector3(0, 1, 0));// look at destination
...
class ThreeContext extends EventManager {
  ...
  constructor(divObj = null) {
    ...
    // init camera
    this._camera = new THREE$1.PerspectiveCamera(27, divObj.clientWidth / divObj.clientHeight, 1, 1000000);
    this._camera.position.z = DEFAULT_FOCUS_DISTANCE;


    // init scene
    this._scene = new THREE$1.Scene();
    this._scene.add(new THREE$1.AmbientLight(0x444444));

    // var axesHelper = new THREE.AxesHelper( 1000 )
    // this._scene.add( axesHelper )

    // adding some light
    const light1 = new THREE$1.DirectionalLight(0xffffff, 0.5);
    ...
    this._renderer = new THREE$1.WebGLRenderer({ antialias: true, alpha: true, preserveDrawingBuffer: true });
    ...
    this._raycastMouse = new THREE$1.Vector2();
  ...
  addStlToMeshCollection(url, options) {
    ...
    loader.load(url, (geometry) => {
      const material = new THREE$1.MeshPhongMaterial({
        specular: 0xffffff,
        shininess,
        side: doubleSide ? THREE$1.DoubleSide : THREE$1.FrontSide,
        color,
        transparent: true,
        opacity,
        wireframe,
...
Object.assign(QuickHull.prototype, {
  ...
  computeExtremes() {
    const min = new THREE$1.Vector3();
    const max = new THREE$1.Vector3();
  ...
  computeInitialHull: (function () {
    ...
    return function computeInitialHull() {
      if (line3 === undefined) {
        line3 = new THREE$1.Line3();
        plane = new THREE$1.Plane();
        closestPoint = new THREE$1.Vector3();
      }
...
function Face() {
  ...
  this.midpoint = new THREE$1.Vector3();
  this.area = 0;
...
Object.assign(Face.prototype, {
  ...
  compute: (function () {
    ...
    return function compute() {
      if (triangle === undefined) triangle = new THREE$1.Triangle();
...
function ConvexGeometry(points) {
  THREE$1.Geometry.call(this);

  this.fromBufferGeometry(new ConvexBufferGeometry(points));
  this.mergeVertices();
}

ConvexGeometry.prototype = Object.create(THREE$1.Geometry.prototype);
...
function ConvexBufferGeometry(points) {
  ...
  this.addAttribute('position', new THREE$1.Float32BufferAttribute(vertices, 3));
  this.addAttribute('normal', new THREE$1.Float32BufferAttribute(normals, 3));
}

ConvexBufferGeometry.prototype = Object.create(THREE$1.BufferGeometry.prototype);
 ...
 */
class MorphologyShapeBase extends THREE$1.Object3D {
  /**
   * @constructor
  ...
  _buildSomaDefault() {
    ...
    if (somaPoints.length === 1) {
      const somaSphere = new THREE$1.Mesh(
        new THREE$1.SphereGeometry(soma.getRadius(), 32, 32),
        new THREE$1.MeshPhongMaterial({ color: 0x000000, transparent: true, opacity: 0.3 }),
      );
    ...
    // when soma is multiple points
      ...
      const centerV = new THREE$1.Vector3(center[0], center[1], center[2]);
      const geometry = new THREE$1.Geometry();

      for (let i = 0; i < somaPoints.length; i += 1) {
        geometry.vertices.push(
          new THREE$1.Vector3(somaPoints[i][0], somaPoints[i][1], somaPoints[i][2]),
          new THREE$1.Vector3(
        ...
        geometry.faces.push(new THREE$1.Face3(3 * i, 3 * i + 1, 3 * i + 2));
      }

      const somaMesh = new THREE$1.Mesh(geometry, new THREE$1.MeshBasicMaterial({
        color: 0x000000,
        transparent: true,
        opacity: 0.3,
  ...
  _buildSomaFromOrphanSections() {
    ...
    try {
      ...
      const somaPolygonPoints = this._morpho.getOrphanSections().map((s) => {
        ...
        return new THREE$1.Vector3(...firstOne)
      });

      // adding the points of the soma (adds values mostly if we a soma polygon)
      for (let i = 0; i < somaPoints.length; i += 1) {
        somaPolygonPoints.push(new THREE$1.Vector3(...somaPoints[i]));
      }
      ...
      somaMesh = new THREE$1.Mesh(geometry, material);
      return somaMesh
    } catch (e) {
  ...
  getTargetPoint() {
    if (this._pointToTarget) {
      ...
      const lookat = new THREE$1.Vector3(this._pointToTarget[0], this._pointToTarget[1], this._pointToTarget[2]);
      lookat.applyAxisAngle(new THREE$1.Vector3(1, 0, 0), Math.PI);
      lookat.applyAxisAngle(new THREE$1.Vector3(0, 1, 0), Math.PI);
      return lookat
    }
...
class MorphologyPolyline extends MorphologyShapeBase {
  ...
  constructor(morpho, options) {
    ...
    this.box = new THREE$1.Box3().setFromObject(this);
  }
  ...
  _buildSection(section) {
    const material = new THREE$1.LineBasicMaterial({
    ...
    for (let i = 0; i < sectionPoints.length; i += 1) {
      geometry.vertices.push(new THREE$1.Vector3(
        sectionPoints[i][0], // x
        sectionPoints[i][1], // y
        sectionPoints[i][2], // z
      ));
    ...
    const line = new THREE$1.Line(geometry, material);
...
const BufferGeometryUtils = {
  ...
  computeTangents(geometry) {
    ...
    if (attributes.tangent === undefined) {
      geometry.addAttribute('tangent', new THREE$1.BufferAttribute(new Float32Array(4 * nVertices), 4));
    }
    ...
    for (var i = 0; i < nVertices; i++) {
      tan1[i] = new THREE$1.Vector3();
      tan2[i] = new THREE$1.Vector3();
    }

    const vA = new THREE$1.Vector3();


    const vB = new THREE$1.Vector3();


    const vC = new THREE$1.Vector3();


    const uvA = new THREE$1.Vector2();


    const uvB = new THREE$1.Vector2();


    const uvC = new THREE$1.Vector2();


    const sdir = new THREE$1.Vector3();


    const tdir = new THREE$1.Vector3();
    ...
    const tmp = new THREE$1.Vector3(); const
      tmp2 = new THREE$1.Vector3();
    const n = new THREE$1.Vector3(); const
      n2 = new THREE$1.Vector3();
    let w; let t; let
      test;
...
class MorphologyPolycylinder extends MorphologyShapeBase {
  ...
  constructor(morpho, options) {
    ...
    this._sectionTubeMaterials = {
      axon: new THREE$1.MeshPhongMaterial({ color: this._sectionColors.axon }),
      basal_dendrite: new THREE$1.MeshPhongMaterial({ color: this._sectionColors.basal_dendrite }),
      apical_dendrite: new THREE$1.MeshPhongMaterial({ color: this._sectionColors.apical_dendrite }),
    };
  ...
  _buildSection(section) {
    ...
    for (let i = startIndex; i < sectionPoints.length - 1; i += 1) {
      const cyl = Tools.makeCylinder(
        new THREE$1.Vector3( // vStart
          sectionPoints[i][0],
          sectionPoints[i][1],
          sectionPoints[i][2],
        ),
        new THREE$1.Vector3( // vEnd
          sectionPoints[i + 1][0],
          sectionPoints[i + 1][1],
          sectionPoints[i + 1][2],
        ),
    ...
    const sectionMesh = new THREE$1.Mesh(sectionGeom, material);
